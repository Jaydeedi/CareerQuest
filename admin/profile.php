...
if (!isset($_SESSION['user_id'])) {
    header('Location: login.php'); // I-redirect sa login page kung hindi naka-login
    exit;
}
...
// ---------------------------------------------
// 1. I-handle ang Form Submission (Update Profile)
// ---------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($new_name) || empty($new_email)) {
        ...
        // Check kung may password update
        ...
        // Check for email change (optional: check if email is already taken by another user)
        $update_fields[] = 'email = ?';
        $params[] = $new_email;
        ...
        if (empty($message)) {
            ...
            if ($stmt->execute($params)) {
                // I-update din ang session name kung nagbago
...
if (!$user) {
    // Dapat hindi mangyari ito kung tama ang logic
...
// Kukunin ang completed lessons count (Code Completed)
...
// Kukunin ang quiz completed count
...
// Kalkulahin ang overall percentage (gamitin natin ang Code Completion)
