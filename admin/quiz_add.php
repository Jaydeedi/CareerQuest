...
$db = getDbConnection();
$message = '';
$page_title = "Add New Quiz Question";
require_once '../includes/header.php'; 

// I-fetch ang lahat ng Lessons (kasama ang Module/Course Title) para sa dropdown
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($lesson_id) || empty($question) || empty($option_a) || empty($option_b) || empty($correct_option)) {
        $message = '<div class="alert alert-danger">All fields (Question, Options A, B, at Correct Answer) are required.</div>';
        ...
        try {
            $sql = "INSERT INTO quizzes (lesson_id, question, option_a, option_b, option_c, option_d, correct_option) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $db->prepare($sql);
            if ($stmt->execute([$lesson_id, $question, $option_a, $option_b, $option_c, $option_d, $correct_option])) {
...
<div class="container mt-5">
    <h2 class="fw-bold mb-4">Add New Quiz Question</h2>
    
    <?php echo $message; ?>

    <form action="quiz_add.php" method="POST" class="card p-4 shadow-sm">
            ...
            <select class="form-select" id="lesson_id" name="lesson_id" required>
                ...
                <?php foreach ($lessons as $lesson): ?>
                    <option value="<?php echo $lesson['id']; ?>" 
                        <?php echo (isset($lesson_id) && $lesson_id == $lesson['id']) ? 'selected' : ''; ?>>
                        <?php echo htmlspecialchars($lesson['lesson_title']) . ' (' . htmlspecialchars($lesson['course_title']) . ' / ' . htmlspecialchars($lesson['module_title']) . ')'; ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <small class="text-muted">Lahat ng quizzes ay konektado sa isang Lesson.</small>
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="correct_option" name="correct_option" required>
                ...
                <?php $options = ['A', 'B', 'C', 'D']; foreach($options as $opt): ?>
                    <option value="<?php echo $opt; ?>" <?php echo (isset($correct_option) && $correct_option == $opt) ? 'selected' : ''; ?>><?php echo $opt; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="d-flex justify-content-between mt-3">
            ...
            <button type="submit" class="btn btn-success">Add Quiz Question</button>
        </div>
    </form>
</div>
